...
ggplot(data = obsDataCSP1, aes(x=obsDataCSP1$date, y=obsDataCSP1$pcpn_in_cnt_ge_1.0, group=obsDataCSP1$name, color=obsDataCSP1$name)) + geom_line() + geom_point() + labs(x = "Year", y = "# days with pcpn >= 1.0 inch", title = "CSP1: Heavy Precipitation (raw data)", color = "Station")

ggplot(data = obsDataGDD[1:75], aes(x=obsDataGDD$date[1:75], y=obsDataGDD$gdd32__cnt_gt_0[1:75], group=obsDataGDD$name[1:75], color=obsDataGDD$name[1:75])) + geom_line() + geom_point() + labs(x = "Year", y = "# growing degree days (annual sum)", title = "CST6: Growing Degree Days - Annual Sum (raw data)", color = "Station")
