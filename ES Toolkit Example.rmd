...
ggplot(data = obsDataCSP1, aes(x=obsDataCSP1$date, y=obsDataCSP1$pcpn_in_cnt_ge_1.0[order(obsDataCSP1$name)], group=sort(obsDataCSP1$name), color=sort(obsDataCSP1$name))) + geom_point() + labs(x = "Year", y = "# days with pcpn >= 1.0 inch", title = "CSP1: Heavy Precipitation (raw data)", color = "Station")
```
```{r echo=FALSE, dpi=96,  message=FALSE, warning=FALSE, error=FALSE}
ggplot(data = obsDataGDD, aes(x=obsDataGDD$date, y=obsDataGDD$gdd32__cnt_gt_0[order(obsDataGDD$name)], group=sort(obsDataGDD$name), color=sort(obsDataGDD$name))) + geom_point() + labs(x = "Year", y = "# growing degree days (annual sum)", title = "CST6: Growing Degree Days - Annual Sum (raw data)", color = "Station")
